    ...
    cds.on('served', () => {
      cds.app?.post('/-/cds/login/token', async (req, res) => {
        try {
          ...
          const q = req.method === 'POST' ? Object.keys(req.body || {}).length ? req.body : Object.fromEntries(new URLSearchParams(
            await new Promise(r => {
                const b = []
                req.on('data', c => b.push(c))
                req.on('end', () => r(Buffer.concat(b).toString()))
              })
            ).entries()
          ) : req.query || {}
          const form = q.refresh_token ? { grant_type: 'refresh_token', refresh_token: q.refresh_token } :
                       q.passcode      ? { grant_type: 'password', passcode: q.passcode } : {}
          const raw = await authToken(cred, { form, subdomain: q.subdomain })
